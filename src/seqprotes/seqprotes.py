...
def seqprotes(f, d, n, m=None, k=100, k_top=10, k_gd=1, lr=5.E-2, r=5, seed=0,
           is_max=False, log=False, info={}, P=None, with_info_p=False,
           with_info_i_opt_list=False, with_info_full=False,
           sample_ext=None, k_rnd=None,  previous_I_y=None, G_hat=None):
    time = tpc()
    ...
    if G_hat is not None:
        ...
        n1 = G_hat[1].shape[2]
        n2 = G_hat[2].shape[1]
        P[0] = P[0].at[:, :n0, :].set(G_hat[0])
        P[1] = P[1].at[:, :, :n1, :].set(G_hat[1])
        P[2] = P[2].at[:, :n2, :].set(G_hat[2])
    ...
    is_new = None
    # update the tensor using previous step results
    run_times = 0
    while True:
        if sample_ext:
        ...
        # if run_times == 0 and previous_I_y is not None, concatenate previous_I_y and I
        if run_times == 0 and previous_I_y is not None:
            # I = jnp.concatenate((previous_I_y, I), axis=0)
            run_times += 1
    ...
    return info['i_opt'], info['y_opt'], info
